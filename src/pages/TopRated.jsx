  ...
  const fetchMovies = async (pageNum) => {
    ...
    try {
      const res = await axios.get(
        `${BASE_URL}/movie/top_rated?api_key=${API_KEY}&page=${pageNum}`
      );
  ...
  return (
    <div className="bg-[#141414] min-h-screen pb-20 pt-24 px-4 md:px-12 text-white font-sans overflow-x-hidden">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-x-2 gap-y-12 isolate">
        {movies.map((m, index) => (
          <div
            key={`${m.id}-${index}`} // Combined key to handle potential duplicate IDs from API pages
          ...
          >
            ...
            <div className="mt-2 group-hover:opacity-0 transition-opacity duration-300">
              ...
              <div className="flex items-center gap-2 mt-0.5">
                 ...
                 <span className="text-[11px] text-gray-500">{m.release_date?.split("-")[0]}</span>
              </div>
            </div>
          </div>
        ))}

        {/* ⏳ SKELETON LOADER (Shown at bottom when loading more) */}
        {loading && (
          <>
            {[...Array(6)].map((_, i) => (
      ...
      {!hasMore && movies.length > 0 && (
        <p className="text-center py-10 text-gray-500 font-medium">You've reached the end of the collection.</p>
      )}
    </div>
  );
};
